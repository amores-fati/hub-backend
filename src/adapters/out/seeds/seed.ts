...
export async function seed(): Promise<void> {
  ...
  // 5. SKILLS
  // Catálogo FECHADO de ~100 tecnologias (espelha o seed de produção). O app
  // apenas SELECIONA desta lista (currículo e vagas via GET /skills), não cria
  // skills novas, então a lista precisa existir previamente no banco.
  const skillNames = [
    'JavaScript',
    'TypeScript',
    'Python',
  ...
  logger.info('- 25 alunos');
  logger.info(`- ${skillNames.length} skills`);
