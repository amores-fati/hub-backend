...
import {
  GenerateVacancyReportPdfCommand,
  IVacancyReportPdfGenerator,
} from '../../../core/ports/vacancy-report-pdf-generator.interface';

interface TableColumn {
  label: string;
  key: keyof GenerateVacancyReportPdfCommand['rows'][number];
  width: number;
  align?: 'left' | 'center';
}

@Injectable()
export class VacancyReportPdfGenerator implements IVacancyReportPdfGenerator {
  ...
  private readonly columns: TableColumn[] = [
    ...
    { label: 'Exclusivo PCD', key: 'isPcd', width: 90, align: 'center' },
    {
  ...
  async generate(command: GenerateVacancyReportPdfCommand): Promise<Buffer> {
  ...
  private renderHeader(doc: PDFKit.PDFDocument, generatedAt: Date): void {
    ...
    doc
      .font('Helvetica-Bold')
      .fontSize(18)
      .text('Relat\u00f3rio de Vagas', this.margin, 66);
  ...
  private renderTable(
    ...
    rows: GenerateVacancyReportPdfCommand['rows'],
  ): void {
    ...
    if (rows.length === 0) {
      doc
        ...
        .fontSize(9)
        .fillColor('#374151')
        .text('Nenhuma vaga encontrada.', this.margin, y + 8);
      return;
    }
  ...
  private renderRow(
    doc: PDFKit.PDFDocument,
    row: GenerateVacancyReportPdfCommand['rows'][number],
    y: number,
    height: number,
  ): void {
    let x = this.margin;
