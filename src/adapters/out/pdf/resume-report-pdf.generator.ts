...
import {
  GenerateResumeReportPdfCommand,
  IResumeReportPdfGenerator,
  ResumeReportPdfRow,
} from '../../../core/ports/resume-report-pdf-generator.interface';

interface TableColumn {
  label: string;
  key: 'interestArea' | 'preference' | 'status';
  width: number;
  align?: 'left' | 'center';
}

@Injectable()
export class ResumeReportPdfGenerator implements IResumeReportPdfGenerator {
  ...
  private readonly studentColumnWidth = 180;
  private readonly columns: TableColumn[] = [
    { label: '\u00c1rea de Interesse', key: 'interestArea', width: 145 },
    { label: 'Prefer\u00eancia', key: 'preference', width: 110 },
    { label: 'Status', key: 'status', width: 81, align: 'center' },
  ];

  async generate(command: GenerateResumeReportPdfCommand): Promise<Buffer> {
  ...
  private renderHeader(doc: PDFKit.PDFDocument, generatedAt: Date): void {
    ...
    doc
      .font('Helvetica-Bold')
      .fontSize(18)
      .text('Relat\u00f3rio de Curr\u00edculos', this.margin, 66);
  ...
  private renderTable(
    ...
    if (rows.length === 0) {
      doc
        ...
        .fontSize(9)
        .fillColor('#374151')
        .text('Nenhum curriculo encontrado.', this.margin, y + 8);
      return;
    }
  ...
  private renderRow(
    doc: PDFKit.PDFDocument,
    row: ResumeReportPdfRow,
    y: number,
    height: number,
  ): void {
    let x = this.margin;

    this.renderStudentCell(doc, row, x, y, height);
    x += this.studentColumnWidth;
  ...
  private renderStudentCell(
    ...
    doc.rect(x, y, this.studentColumnWidth, height).stroke('#e5e7eb');

    doc
      .font('Helvetica-Bold')
      .fontSize(8)
      .fillColor('#111827')
      .text(row.studentName, x + this.cellPadding, y + 6, {
        width: this.studentColumnWidth - this.cellPadding * 2,
      });

    const nameHeight = doc.heightOfString(row.studentName, {
      width: this.studentColumnWidth - this.cellPadding * 2,
    });
  ...
  private calculateRowHeight(
    ...
    const studentNameHeight = doc.heightOfString(row.studentName, {
      width: textWidth(this.studentColumnWidth),
    });
    ...
    return Math.max(
      this.rowMinHeight,
      studentNameHeight + cpfHeight + this.cellPadding * 2 + 4,
      contentHeight + this.cellPadding * 2,
    );
  }
  ...
  private allColumns(): Array<{
    ...
    return [
      { label: 'Aluno', width: this.studentColumnWidth },
      ...this.columns,
    ];
  }
